...
/**
 * The permission-check enforcement point for numbering's own REST layer — same pattern as {@code
 * com.eudext.erp.workflow.internal.web.WorkflowAccessControl}: IAM's own access-control machinery is module-internal
 * (ARCH-1), so business modules read the JWT subject off {@code SecurityContextHolder} and check it against {@link
 * PermissionApi} directly.
 */
@Component
public class NumberingAccessControl {

    private final PermissionApi permissionApi;

    public NumberingAccessControl(PermissionApi permissionApi) {
