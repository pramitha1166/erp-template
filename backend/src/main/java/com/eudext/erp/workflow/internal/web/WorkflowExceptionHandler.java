...
/** Translates workflow's internal exceptions into the REST layer's error responses. */
@RestControllerAdvice(basePackages = "com.eudext.erp.workflow")
public class WorkflowExceptionHandler {
    ...
    @ExceptionHandler(NotAssignedApproverException.class)
    public ResponseEntity<ApiError> notAssignedApprover(NotAssignedApproverException e) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiError.of(e.getMessage()));
    }

    @ExceptionHandler(MandatoryCommentMissingException.class)
    public ResponseEntity<ApiError> mandatoryCommentMissing(MandatoryCommentMissingException e) {
    ...
    public ResponseEntity<ApiError> illegalState(IllegalWorkflowStateException e) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ApiError.of(e.getMessage()));
    }

    @ExceptionHandler(NoApproverResolvedException.class)
    public ResponseEntity<ApiError> noApproverResolved(NoApproverResolvedException e) {
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(ApiError.of(e.getMessage()));
    }
